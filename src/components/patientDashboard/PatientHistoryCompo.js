...
const PatientHistoryCompo = (props) => {
  ...
  return (
    <div className="grid grid-cols-4">
      <div>
        <h1>{props.prescription.date}</h1>
      </div>
      <div className="flex">
        <h1>Dr.</h1>
        <h1>{props.prescription.docter_name}</h1>
      </div>
      <div>
        <h1>{props.prescription.daignosis}</h1>
      </div>
      <Link
        to="/patient/prescription"
        // onClick={props.setPrescriptionID(props.prescription.id)}
      >
